...
async function fetchText(url: string): Promise<{ body: string; contentType: string | null }> {
  const res = await fetch(url, { headers: { "User-Agent": "wog-notifier/1.2 (+netlify)" } });
  ...
  try {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      if ((contentType ?? "").includes("json") || body.trim().startsWith("{")) {
        items.push(...parseSamJson(body, src).filter(x => matchesKeywords(x.title + " " + x.summary, kw) && isFresh(x.pub)));
      } else {
    ...
    if (!unique.length) return { statusCode: 200, body: "No fresh items." };
    unique.sort((a,b)=> new Date(b.pub).getTime()-new Date(a.pub).getTime());
    const li = unique.map(x => `<li><a href="${x.link}">${escapeHtml(x.title)}</a><br/><small>${x.pub} – ${escapeHtml(x.source)}</small></li>`).join("");
    const html = `<p>Nowe ogłoszenia:</p><ul>${li}</ul>`;
    await sendMail(`${MAIL_SUBJECT_PREFIX} ${unique.length} nowych ogłoszeń`, html);
    return { statusCode: 200, body: `Sent ${unique.length} items` };
  } catch (err) { return { statusCode: 500, body: (err as Error).message }; }
};
